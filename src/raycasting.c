			...
			int *side, double *hit_x, Game_env *game)
{
	...
	*hit_x -= floor(*hit_x);  /* Get fractional part of the hit location */
	...
	if (*side == 0)
		...
		return ((map_y - player_y + (1 - step_y) / 2) / sin(ray_angle));
>>>>>>> e3187086a8d494008c0fd76a78ac38e2c63e239a
}
...
void render_walls(SDL_Renderer *renderer, Game_env *game, Player player)
{
	...
	{
	/* Calculate ray angle with FOV spread */
	double ray_angle = player.angle - (FOV / 2) * (M_PI / 180.0) +
				(ray * (FOV * (M_PI / 180.0)) / NUM_RAYS);

	int side;
	double hit_x;

	/* Cast the ray and get the distance to the wall */
	double distance_to_wall = cast_ray(player.x, player.y, ray_angle,
					&side, &hit_x, game);
